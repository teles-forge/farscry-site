...
## Smart paste — Cmd+V becomes image-aware

The fastest workflow: configure your terminal so Cmd+V automatically runs
farscry when the clipboard contains an image.

Run `farscry setup` and choose "Configure smart paste" when prompted.

farscry creates `~/.farscry/smart-paste.sh` and shows the key binding
...
After setup: take any screenshot, press Cmd+V in your terminal, farscry
detects the image, sends VASP context to your configured agent. Done.
